...
def preprocess_data():
    ...
    try:
        # Get preprocessing parameters from form
        target_column = request.form.get('target_column', 'RR')
        bins_str = request.form.getlist('bins')
        labels = request.form.getlist('labels')
        
        # Convert bins to float, handling empty values and errors
        bins = []
        for bin_val in bins_str:
            if bin_val:
                try:
                    bins.append(float(bin_val))
                except ValueError:
                    flash(f"Warning: '{bin_val}' is not a valid number and was ignored.")
        ...
        try:
            ...
            flash(f'Error during target definition: {str(e)}')
            return redirect(url_for('upload_file'))
